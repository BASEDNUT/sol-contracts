...
contract MockSchemaRegistry {
    ...
    function uidFor(string calldata schema, ISchemaResolver resolver, bool revocable)
...
contract MockEAS {
    ...
    constructor(address registry_) { registry = ISchemaRegistry(registry_); }

    function getSchemaRegistry() external view returns (ISchemaRegistry) { return registry; }
...
contract MockPipsBuyer {
    ...
    constructor(address usdc_, address pips_) { usdc = MockUSDC(usdc_); pips = MockERC20(pips_); }
...
contract MockSwapRouter {
    ...
    function swapExactTokensForTokens(
        uint256 amountIn, uint256 amountOutMin, address[] calldata, address to, uint256
...
contract MockLpRouter {
    ...
    constructor(MockPair pair_) { pair = pair_; }
